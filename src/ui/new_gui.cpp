...
{
    /* Exactly what the original's RandomizePatchButton::click() does
     * (src/gui/widgets.cpp:143) and nothing else: hand RANDOMIZE to the synth and
     * let the patch the generator produced stand, macros included.
     *
     * This used to snapshot the 8 performance macros, wait for the message to
     * drain, then relocate the random patch's macro references onto pool slots
     * and write the saved macros back -- so RND here produced a different patch
     * from RND in the original GUI, off the same generator. The refresh below
     * picks the new values up, as it does for any other external change. */
